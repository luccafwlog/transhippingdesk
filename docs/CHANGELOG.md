...
- **Pós-auditoria PRs #405/#406:** RBAC de COD/transbordo e de escrita de clientes/contatos passou a ser reforçado no banco (`can_edit_voyages()`/`can_edit_customers()`, migration `208`), não só escondido na UI; leitura de recebíveis do cliente migrou para RPC dedicada (`get_customer_receivables`, migration `209`) para não confundir "sem recebível" com "sem permissão"; corrigido overload duplicado de `omit_voyage_escala`; timeline da ficha do cliente passou a incluir pagamentos locais e a ser invalidada por mutação de contato; abas Visão Geral/Financeiro/Histórico da ficha e o card de status Baplie do B/L Cockpit distinguem carregando/erro/vazio; agregados (saldo, pendências) paginam até esgotar em vez de truncar. *(auditoria `docs/archive/audits/2026-07-19-pos-merge-audit-pr405-406.md`)*
