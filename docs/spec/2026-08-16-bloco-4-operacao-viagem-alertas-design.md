...
1. Alerta é pendência coletiva; Notificação Interna é entrega individual no
   ...
   departamental: Documentação ou Equipamentos.
...
- Viagem somente exportação não pode ter POD. Escala mista sem POD pode existir:
...
| BL esperado por POL/POD | Alerta crítico + Notificação para todos de Documentação | Viagem | Fecha com cobertura mínima de cada POL e POD; reabre com nova expectativa ou remoção de BL | D−7 e alterações materiais; /viagens/:voyageId |
| Baplie ausente | Alerta crítico + Notificação para Documentação | Viagem | Fecha com importação válida; reabre se o arquivo for invalidado/removido | D−7 e invalidação; /baplie filtrado |
| Cobertura documental Baplie/BL | Alerta crítico + Notificação genérica para Documentação | Viagem | Fecha com todas as rotas confrontáveis e sem divergência; novo ciclo após recorrência | Imediato quando rotas cobertas; override em D−7; /baplie filtrado |
| CE Mercante ausente | Alerta crítico + Notificação para Documentação | Viagem | Fecha quando todos os BLs de importação existentes têm CE; reabre com nova pendência | D−5 e alterações materiais; /viagens/:voyageId |
| ATD e prazos de agência | Nenhum evento novo | ADR do #524 | Mantém contrato do ADR | Não duplicar no #523 |
| Exportação pós-ATD | Alerta normal + Notificação para todos de Equipamentos | Escala | Fecha quando tipos esperados têm vínculo; remoção reabre | ATD e alterações; viagem com escala selecionada |
| Erro de importação de planilha | Feedback transacional, sem alerta | — | — | Ação na própria tela |
| Depots e serviços cadastrais | Nenhum evento próprio | — | — | Validação síncrona |
...
O Baplie só existe para importação e a importação já exclui portos sem nossa
agência. Não comparar diretamente Baplie com Chegadas e Saídas.
...
- mesmo com rotas sem BL, forçar a checagem;
- rotas sem BL entram como pendência no mesmo alerta;
- rotas com BL são avaliadas quanto à divergência;
...
Uma escala mista participa dos fluxos de importação e exportação. O alerta
abre após ATD confirmado e é por escala:
...
1. Campo e validações do primeiro porto brasileiro externo e seu ETA.
...
6. Detector server-side de Baplie ausente.
7. Reconciliação Baplie/BL com regra normal e override D−7.
...
usar tabelas existentes e não criar tabela por container ou BL.
