...
O que sobra são **8 achados residuais**, nenhum crítico: um de severidade média
(privilégio de escrita entre departamentos internos), quatro baixos e três
informativos.

| # | Achado | Classe | Severidade |
|---|---|---|---|
| A-01 | Departamento interno é fronteira de UI, não de banco, em 5 módulos | 2 | **Média** |
| A-02 | `portal_ship_schedule()` concedida a `anon` sem necessidade | 1 / 3 | Baixa |
...
### A-01 — Departamento interno é fronteira de UI, não de banco (Média)
...
### A-02 — `portal_ship_schedule()` concedida a `anon` (Baixa)
...
- Nas rotas internas (`/manifestos/:blId`, `/clientes/:cnpj`, `/viagens/:voyageId`)
  ...
  alinhou a RLS a isso. A restrição é de escrita (ver A-01).
