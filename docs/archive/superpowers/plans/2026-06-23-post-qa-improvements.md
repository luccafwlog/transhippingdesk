...
- [x] Done — re-audited: the only remaining native `window.confirm` was `PortalBilling.tsx:161` (the others on that earlier list already use `ConfirmDialog`; the grep had matched the styled `confirm(...)` calls too). Migrated it to `useConfirm` (danger tone) and added confirm/cancel-gating tests in `PortalBilling.test.tsx`. `grep -rn "window.confirm" src` now returns nothing.
