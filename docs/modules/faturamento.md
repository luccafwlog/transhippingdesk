...
| Validação · emitir invoice individual | B/L `ready_for_billing`, não faturado, com cliente e, para Granito, CE Mercante | `handleIssueSingleInvoice` | `issueOperationalInvoice` roteia Granito por `calculateAndIssueGraniteInvoice` | RPC local ou `create_invoice_from_granite_bls`, conforme o modo | Invalida operações, invoices, B/Ls e resumo | RPC revalida estado, cliente, vínculos e permissões; Granito sem CE não pode ser emitido | **Código:** `src/components/billing/ValidacaoTab.tsx`, `src/services/graniteBillingWorkflow.ts` · **Teste:** `src/services/__tests__/reviewBillingAutomation.test.ts` |
