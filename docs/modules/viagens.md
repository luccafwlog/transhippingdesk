...
- `src/components/voyages/VoyageRail.tsx` é uma faixa horizontal com rolagem, sempre visível acima do detalhe (desktop e mobile). Cada card mostra armador, navio/viagem, estado de conciliação, as escalas brasileiras com seus ETAs (`escalasBrasileiras`, de `buildVoyageRailItems`) e selos dos módulos presentes na viagem (Container, Carga solta, Veículos, Vazios, Granito); portos no exterior e contagens de B/L/container não aparecem no card — origem/destino completos seguem disponíveis para a busca (`viagensFilters.ts`). A seleção navega para `/viagens/:voyageId`. Setas nas pontas aparecem só quando há conteúdo fora da viewport para aquele lado; ver nota editorial de 2026-08-05 no [ADR 0012](../adr/0012-viagens-master-detail-rota-dedicada.md).
...
- **Visão geral:** KPIs, planejamento por POD/POL, edição/exclusão de agenda, cards contextuais e timeline. O planejamento registra, por escala, ETA/ATA, ETB/ATB e ETD/ATD, mantendo previsão e realização em campos distintos.
...
| Omitir escala | Usuário ativo (não exige Admin — ADR 0046); escala ativa com importação; Porto de Transbordo preenchido | Botao de alerta na grade em `VoyageVisaoTab` + `OmitEscalaModal` | Cria a omissão e o motivo no registro global e cria a disposição `transshipment` para os B/Ls afetados. **Divergência conhecida:** o modal também coleta navio, armador, viagem, ETD e ETA, mas a RPC vigente os descarta — ver Notas e divergências | `voyage_omissions`, `bl_transshipments`, `audit_logs.field_name='omitted'`, `portal_notifications.type='transshipment'` | Invalida transbordos, POD schedules, timeline, viagens, B/Ls e Line-Up; envia a notificação inicial da omissão | RPC valida usuário ativo, `changed_by=auth.uid()`, portos não vazios e viagem existente | `src/components/voyages/OmitEscalaModal.tsx`; `src/hooks/useTransshipments.ts`; `src/services/transshipments.ts`; migrations `174`/`175`/`201`; testes `voyageOmissionsMigration`, `voyageOmissionGlobalMigration`, `transshipments` |
| Complementar Informações de Transbordo | Omissão registrada; usuário autenticado | `TransshipmentInfoCard` na Visão da Viagem | `useUpdateVoyageOmission` chama `update_voyage_omission`; campos desconhecidos permanecem `—`; todos os B/Ls herdam a leitura global | Campos `onward_*` e `reason` em `voyage_omissions`; auditoria da viagem e dos B/Ls | Invalida transbordos, timeline e B/Ls; não cria `portal_notifications` | RPC valida usuário ativo, `changed_by=auth.uid()` e omissão existente | `src/components/voyages/TransshipmentInfoCard.tsx`; `src/hooks/useTransshipments.ts`; migration `201`; testes `TransshipmentInfoCard`, `voyageOmissionGlobalMigration` |
| Restaurar transbordo após COD | Omissão registrada e B/L em COD | `TransshipmentPanel` | `useSetBlDisposition().setTransshipment` chama `set_bl_transshipment` e restaura o POD original; os dados de navio permanecem globais e somente leitura no B/L | `bl_transshipments.disposition`, `bls.pod` e `audit_logs` do B/L | Invalida transbordos, B/Ls e viagens | RPC valida usuário ativo e linha existente | `src/components/voyages/TransshipmentPanel.tsx`; `src/services/transshipments.ts`; migrations `174`/`201` |
| Marcar COD | Omissao registrada e B/L afetado | `BlTransshipmentCard` na ficha `/manifestos/:blId` | `useSetBlDisposition().setCod` chama `set_bl_cod`; limpa campos de navio terceiro e altera `bls.pod` para o porto de descarga | `bl_transshipments`, `bls.pod`, `audit_logs`, `portal_notifications.type='transshipment'` | Invalida transbordos, B/Ls e viagens | A Viagem lista a disposição como leitura com link para o B/L; RPC valida usuario ativo e linha existente | `src/components/bl/BlTransshipmentCard.tsx`; `src/services/transshipments.ts`; migration `174` |
...
| Importação rápida no contexto da viagem | Usuário autenticado; tipo disponível na aba; Equipamentos limitado a Veículos e Vazios EXP | `VoyageImportActions` | Abre parsers/previews com `voyageId` travado. O código expõe Baplie, B/L, CE Mercante, Manifesto BB, Veículos, Vazios IMP, Granito e Vazios EXP conforme a aba; o papel Equipamentos recebe somente os dois tipos do seu escopo | Conforme o importador proprietário | Usa arrays literais como `['bls']`, `['voyages']`, `['lineup-tv-v3']`, `['baplie-staging', voyageId]`, `['vehicles']` | BB e Veículos oferecem planilhas-modelo; a atomicidade varia por importador | `src/components/shared/VoyageImportActions.tsx`; `src/components/shared/FileImportModal.tsx` |
...
- **Omitir escala descarta os dados de transbordo (P0).** `OmitEscalaModal` coleta navio, armador, viagem, ETD e ETA, mas a definição vigente de `omit_voyage_escala` (migration `215`) mantém os parâmetros `p_onward_*` na assinatura e não os grava — o corpo foi copiado da `174`/`206`, perdendo o que a `201` implementou. O operador precisa redigitar em **Complementar**. A mesma migration reintroduziu o guard `v_omitted = v_discharge`, rebloqueando a omissão de POD único que a `177` (issue #355) havia liberado. Correção na Task 1 de [`docs/plans/2026-08-18-transbordo-cod-correcoes.md`](../plans/2026-08-18-transbordo-cod-correcoes.md); levantamento completo em [`docs/archive/audits/2026-08-18-revisao-transbordo-cod.md`](../archive/audits/2026-08-18-revisao-transbordo-cod.md).
- **Escala omitida ainda aparece no Line-Up como chegada pendente.** `src/services/lineup.ts` não filtra nem marca `omitted`, embora a flag esteja disponível; como escala omitida nunca recebe ATA/ATD, a linha permanece indefinidamente. Correção na Task 10 do mesmo plano.
