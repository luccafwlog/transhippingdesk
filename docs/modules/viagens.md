...
| Importação rápida no contexto da viagem | Usuário autenticado; tipo disponível na aba; Equipamentos limitado a Veículos e Vazios EXP | `VoyageImportActions` | Abre parsers/previews com `voyageId` travado. Na aba Importação, a barra agrupa Baplie EDI, B/L container, B/L carga solta e CE Mercante, depois Veículos e Vazios IMP; CE Mercante continua sendo um alvo único sobre `bls`, e os dois B/Ls abrem modais distintos | Conforme o importador proprietário | Usa arrays literais como `['bls']`, `['voyages']`, `['lineup-tv-v3']`, `['baplie-staging', voyageId]`, `['vehicles']` | B/L carga solta usa documentos `.pdf`/`.docx` com a viagem travada; BB e Veículos oferecem planilhas-modelo; a atomicidade varia por importador | `src/components/shared/VoyageImportActions.tsx`; `src/components/shared/BlDocumentImportModal.tsx`; `src/components/shared/FileImportModal.tsx` |
