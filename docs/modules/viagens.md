...
| Importação rápida no contexto da viagem | Usuário autenticado; tipo disponível na aba | `VoyageImportActions` | Abre parsers/previews com `voyageId` travado. O código expõe Manifesto BB, Vazios IMP, Veículos, Baplie e B/L; Manifesto CNTR foi removido. A reorganização de ordem e a entrada contextual de CE Mercante pertencem à WS4. | Conforme o importador proprietário | Usa arrays literais como `['bls']`, `['voyages']`, `['lineup-tv-v3']`, `['baplie-staging', voyageId]`, `['vehicles']` | BB e Veículos oferecem planilhas-modelo; a atomicidade varia por importador | `src/components/shared/VoyageImportActions.tsx`; `src/components/shared/FileImportModal.tsx` |
