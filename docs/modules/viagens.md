...
| Navegar para manifestos, carga solta, Granito, vazios e Baplie | Card habilitado quando há dados; Baplie aparece para divergências | `NavigationCard` / botão “Resolver divergências” | `navigate` acrescenta `?voyage=<id>` | Nenhuma | A tela destino decide se consome o parâmetro | `/carga-solta` não lê hoje o parâmetro; `/vazios` redireciona sem preservar explicitamente a query; o Embarque de Vazios trava a viagem quando recebe `?voyage=<id>` | `src/components/voyages/VoyageCard.tsx`; `src/App.tsx`; `src/pages/Manifestos.tsx`; `src/pages/CargaSolta.tsx`; `src/pages/EmbarqueVazios.tsx` |
