...
- `src/components/voyages/VoyageRail.tsx` é uma faixa horizontal com rolagem, sempre visível acima do detalhe (desktop e mobile). Cada card mostra rota POL→POD, B/Ls, containers distintos, próxima escala e estado de conciliação. A seleção navega para `/viagens/:voyageId`. Setas nas pontas aparecem só quando há conteúdo fora da viewport para aquele lado; ver nota editorial de 2026-08-05 no [ADR 0012](../adr/0012-viagens-master-detail-rota-dedicada.md).
