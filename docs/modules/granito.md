...
- `/faturamento` → `Validação` agrega B/Ls de granito à visão operacional,
  calcula em lote, promove para `ready_for_billing` e expõe a ação manual
  `Emitir`.
...
|---|---|---|---|---|---|---|---|
| `/granito` · abrir import e selecionar viagem | Sessão interna; viagem existente para confirmar | `Granite`; estado `uploadOpen`, `voyageId`; `useVoyageOptions` | A seleção só prepara os argumentos do parser/importador | Leitura de viagens pelo hook | Estado local; após sucesso invalida `['granite-bls']` e `['voyages']` | Sem viagem, arquivo ou usuário o botão permanece desabilitado | **Código:** [`Granite.tsx`](../../src/pages/Granite.tsx), [`useBls.ts`](../../src/hooks/useBls.ts) |
| `/viagens/:voyageId` · importar Granito | Viagem e usuário já definidos; arquivo com B/Ls válidos | `VoyageImportActions`; `FileImportModal` | `parseGraniteManifestFile` → `importGraniteManifest` | `granite_manifests`, `granite_bls` | Invalida `['voyages']` e `['granite-manifests']` | Erro de parse por arquivo; falha de persistência interrompe o lote | **Código:** [`VoyageImportActions.tsx`](../../src/components/shared/VoyageImportActions.tsx), [`FileImportModal.tsx`](../../src/components/shared/FileImportModal.tsx) |
...
| `/granito` · calcular taxas | B/L existente; rates ativas/vigentes; peso real disponível | `handleCalculateCharges` | `calculateGraniteBlCharges` apaga snapshot anterior, filtra vigência e calcula quantidade/subtotal | DELETE/INSERT em `granite_bl_charges`; UPDATE de `granite_bls.charge_status` | Invalida `['granite-bls']` e `['voyages']`; sem cliente abre modal de linhas | Sem rates grava `calculated` e retorna vazio; falha após DELETE pode deixar snapshot vazio | **Código:** [`Granite.tsx`](../../src/pages/Granite.tsx), [`graniteCharges.ts`](../../src/services/graniteCharges.ts) |
| `/faturamento` · promover Granito a pronto | Linha selecionada; ação em lote `ready` | `ValidacaoTab.runGraniteBatch` | `markGraniteBlReady` | UPDATE direto `granite_bls.charge_status='ready_for_billing'` | Invalida `queryKeys.charges.operations()` e caches de invoices/B/Ls | Erro é agregado por B/L no resultado do lote | **Código:** [`ValidacaoTab.tsx`](../../src/components/billing/ValidacaoTab.tsx), [`chargeOperationsService.ts`](../../src/services/charges/chargeOperationsService.ts) |
| `/granito` · autoemitir invoice | B/L com `client_id`; usuário; snapshot positivo e elegível | `handleCalculateCharges` | `calculateGraniteBlCharges` → `createInvoiceFromGraniteBls` → `persistPixPayload` | RPC `create_invoice_from_granite_bls`; `invoices`, `invoice_items`, `invoice_granite_bls`, `audit_logs`; UPDATE `granite_bls` | Invalida `['invoices']`; toast de sucesso | A RPC exige admin, `ready_for_billing`, cliente único, total BRL positivo e ausência de invoice ativa | **Código/Suspeita:** [`Granite.tsx`](../../src/pages/Granite.tsx), [`billing.ts`](../../src/services/billing.ts), [`20260528134131_fix_granite_invoice_cancel_reissue.sql`](../../supabase/migrations/20260528134131_fix_granite_invoice_cancel_reissue.sql) |
...
| `/faturamento` · abrir invoice ligada | Invoice retornada pela lista genérica | `InvoicesTable` → `InvoiceDetailModal` | `useInvoiceDetail` → `listInvoiceDetails`/RPC `list_invoice_details` | SELECT em `invoices`, `invoice_items`, `payments`; vínculo Granite permanece em `invoice_granite_bls` | Query `['invoice-detail', id]`; impressão e pagamento usam a superfície comum | Não existe link direto em `/granito`; lista/detalhe genéricos não consultam `invoice_granite_bls` para metadados do B/L | **Código:** [`Faturamento.tsx`](../../src/pages/Faturamento.tsx), [`InvoicesTable.tsx`](../../src/components/billing/InvoicesTable.tsx), [`billing.ts`](../../src/services/billing.ts), [`020_billing_hybrid_workflow.sql`](../../supabase/migrations/020_billing_hybrid_workflow.sql) |
...
- **Estado de cálculo:** `calculateGraniteBlCharges` grava `calculated`;
  `markGraniteBlReady` promove explicitamente a `ready_for_billing`; a RPC de
  emissão exige esse estado e termina em `invoiced`. No caminho prático de
  faturamento, cálculo e promoção precisam acontecer antes da emissão.
...
- **Taxas Locais:** a migration
  [`20260523120000_taxas_locais_granito.sql`](../../supabase/migrations/20260523120000_taxas_locais_granito.sql)
...
- A RPC mais recente em
  [`20260528134131_fix_granite_invoice_cancel_reissue.sql`](../../supabase/migrations/20260528134131_fix_granite_invoice_cancel_reissue.sql)
...
- **Código — `invoice_type` não é informado pela RPC Granite:** a função atual
  ...
  [`20260529100000_local_billing_ledger_phase1.sql`](../../supabase/migrations/20260529100000_local_billing_ledger_phase1.sql)
