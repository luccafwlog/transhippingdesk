...
| `/portal/billing` — exportar resultado | Aba ativa; página filtrada carregada | `handleExport` | `exportPortalLocalInvoicesWorkbook` ou `exportPortalDemurrageWorkbook` | Download XLSX local com `@e965/xlsx` | Exporta as linhas da página atualmente carregada; sem mutação/cache | A página não aguarda nem apresenta toast de falha do export | **Teste:** `src/pages/__tests__/PortalBilling.test.tsx`; **Código:** `src/services/exports.ts` |
