...
17. **Importar B/L não toca faturamento direto.** `import_bl_freight_transactional` persiste `bl_freight_lines` e `bl_emission_date`; não insere nem recalcula `charge_calculations`, invoices ou ledger. Quando há cálculo/invoice, peso e containers ficam bloqueados e apenas campos comerciais/frete seguem corrigíveis.
...
- Importar B/L: `src/services/__tests__/blParser.test.ts`, `src/services/__tests__/blFreightImport.test.ts`, `src/services/__tests__/blFreightLinesMigration.test.ts`, `src/services/__tests__/mercanteEdiGenerator.test.ts` e `src/components/shared/__tests__/BlImportModal.test.tsx`.
