...
> **Divergência transitória — ADR 0025:** a rota continua listando B/Ls CNTR,
> mas as ações de importar Manifesto CNTR e gerar EDI Mercante deverão ser
> removidas. A fonte documental vigente é o arquivo de B/L; permanecem na lista
> as ações de exportar, importar CE Mercante e importar B/L.

- `src/pages/Manifestos.tsx` lista B/Ls de container com paginação, seleção em massa, resumo e filtros por texto, viagem, POL, POD, revisão, financeiro, taxas locais e perfil de carga. O código ainda expõe `Gerar EDI Mercante` e concentra o upload legado de Manifesto CNTR em `src/components/shared/UploadManifestModal.tsx`; ambas as entradas divergem da interface aprovada e devem ser removidas, tornando superada a solicitação anterior de apenas uniformizar a cor do botão CNTR.
...
- Para listagens e reconciliação por nome, o consignatário curto termina na natureza jurídica (`LTDA`, `S.A.`, `EIRELI`, `EI`, `MEI`, `SLU`, `EPP`, `ME`, incluindo combinações); sem marcador reconhecido, usa a primeira linha não vazia. O bloco completo permanece intacto para EDI e auditoria.
- Pela ADR 0025, `Laden on Board` deverá persistir o ATD do POL. Entre B/Ls da mesma Viagem e POL prevalece automaticamente a data mais antiga. ETD e ATD permanecem distintos; telas sem coluna própria mostram ATD em verde na célula de ETD.
...
- O código ainda expõe “Importar IMO/OOG” e mantém `containerFlagsImport.ts`, mas esse fluxo deve ser integralmente retirado — botão, modal, parser, serviço e testes exclusivos. O Baplie EDI é a fonte física única desses atributos; a resolução operacional de divergências do Baplie permanece disponível.
...
| `/containers` — importar IMO/OOG — **fluxo condenado** | Planilha válida; match B/L+container | Modal em `Containers` | O código ainda deduplica, atualiza matches e audita; a interface e toda a implementação exclusiva deverão ser removidas | `bl_containers`, `audit_logs` | Deixa de existir como importação independente | Baplie EDI passa a ser a fonte física única; sua resolução de divergências permanece | **Código**, **Divergência documentada**: `src/pages/Containers.tsx`; `src/services/containerFlagsImport.ts` |
