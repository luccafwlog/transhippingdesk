# Clientes
> **Status:** ativo · **Atualizado:** 2026-06-18 · **Rotas:** `/clientes`, `/clientes/:cnpj`
...
- **Provisionamento de portal:** na ficha, o admin define `portal_email` + senha (mín. 8 caracteres). `savePortalMutation` chama o RPC `upsert_customer_portal_account` e em seguida `provisionPortalAuthUser`, que invoca a Edge Function `provision-portal-user` com `{ customer_portal_account_id, portal_email, password }`. A function cria/atualiza o usuário Supabase Auth e grava `auth_user_id` na conta. Ver [Portal do Cliente](portal-cliente.md).
