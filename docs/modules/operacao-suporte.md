...
Ações de nível-cliente alteram primeiro `customers`/`customer_contacts` e depois chamam `recomputeBlReviewGate` para cada B/L vinculado do grupo. O provisionamento do Portal ocorre no Console dedicado, por convite de uso único e ativação pelo cliente; a Revisão não cria senhas nem exibe credenciais.
...
| Criar e selecionar cliente no drawer | Nome e CNPJ válido de 14 posições; permissão de escrita | Formulário “Ou cadastre” | `createCustomer`; colagem/digitação normalizada e, em duplicidade, busca cadastro existente e o seleciona | `customers`, opcionalmente `customer_contacts` | Invalida `['customers']`; seleção só é vinculada ao salvar revisão | CNPJ inválido, duplicidade não resolvida ou `42501` geram feedback | **Código:** `src/pages/Revisao.tsx`, `src/services/customers.ts` |
