...
Não há lock otimista nessa atualização. A proteção efetiva para `role` e `active` é administrativa no banco, incluindo o trigger de `supabase/migrations/20260530102906_fix_user_profile_privilege_escalation.sql`.
...
`supabase/migrations/20260619130000_review_gate_hardening.sql` é a definição posterior do contrato:
