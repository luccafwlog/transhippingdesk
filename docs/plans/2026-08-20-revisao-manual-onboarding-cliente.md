...
- Create: `supabase/migrations/321_review_customer_group_onboarding.sql` —
...
- Create: `src/services/__tests__/reviewCustomerGroupMigration.test.ts` — teste
  de contrato SQL da migration 321.
...
O teste deve carregar a migration 321 e exigir os seguintes contratos:
...
Expected: FAIL com ausência da migration 321 e das funções novas.
...
Na migration 321, substituir o wrapper vigente de
...
```bash
git add supabase/migrations/321_review_customer_group_onboarding.sql src/services/__tests__/reviewCustomerGroupMigration.test.ts
...
```bash
git add src/services/__tests__ supabase/migrations/321_review_customer_group_onboarding.sql
...
321 no banco descartável. Nunca apontar esse passo para produção.
