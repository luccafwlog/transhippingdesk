...
| A6 — volume por cliente | Precisa de investigação | Ausência de teto por cliente/execução é confirmada; “sem teto” na régua descreve continuidade até quitação e não decide sozinho quantas mensagens simultâneas são aceitáveis. Medir carteira × contatos e decidir cap/agregação/aceitação explícita antes de envio em massa. | S06 / D11 |
...
| A7 | Precisa de investigação | Aviso de bounce usa canal transacional do Portal, fora da chave; CONTEXT chama esse aviso transacional, ADR 0059 não o enumera nas exceções. Decidir contrato antes de alterar. | S06, decisão D04 |
...
| F4 — autoridade server-side | Aceito | ADR 0026 aceita validar autoconsistência do payload, sem recalcular a tarifa no servidor. A mudança proposta para execução durável exige decisão explícita e supersessão parcial. | S08-B / D02 |
...
- [ ] Para Demurrage, habilitar `demurrage_billing` automático apenas após S08-B. Sem D02, salvar pendência e permitir retomada autenticada do cálculo pelo operador, com indicação clara de que recuperação totalmente automática continua bloqueada por decisão, não concluída.
...
- [ ] Para A6, medir faturas elegíveis × contatos: fixture de 12 faturas e 3 contatos produz previsão de 36 entregas, sem enviá-las. Apresentar D11 antes de ativação em massa. Se cap for aprovado, persistir adiamento por cliente com ordenação justa e retomada sem perda; se agregação for escolhida, definir um documento por cliente com links/identidades das faturas e dedup por ciclo, sem misturar clientes. Se manter volume, registrar aceitação explícita e teto do provedor verificado. Implementar somente o ramo aprovado em PR própria com teste de excesso e retomada.
...
- [ ] Resolver A7: se a decisão mantiver aviso transacional fora da chave, registrar exceção explícita na ADR 0059 e teste; se a decisão incluir bounce na chave, condicionar apenas aviso ao cliente, mantendo supressão, reparo e alerta interno. Convite/reset/segurança mantêm isenções já aceitas. Nenhuma mudança enquanto D04 não for decidida.
...
- [ ] Fazer readiness na transação de criação/claim de comunicação e revalidar antes do provedor. Função STABLE de leitura sozinha não serializa alteração concorrente; lock/revisão do agregado deve proteger a decisão persistida, sem segurar lock durante HTTP.
...
- [ ] Formalizar supersessão parcial da ADR 0026 se D02 aprovada: servidor resolve tarifa/acordo/override e valida conjunto de containers, datas e versão do preview. Tarifa global usa dia do cálculo, acordo usa sua âncora vigente de domínio; não substituir tudo por data de descarga. Incluir fixtures para precedência de acordo e override de free time/tarifa.
- [ ] Criar leitura de preview autoritativa e emissão com revisão esperada; payload adulterado/stale retorna conflito e exige nova conferência. Interno e worker usam o mesmo núcleo de cálculo, wrappers distintos de autorização. Sem D02, manter veto atual e marcar essa entrega condicional, sem afirmar fila autônoma de Demurrage concluída.
...
- [ ] Implementar a política D05 em um núcleo: recomendação é saldo positivo continuar parcialmente pago; se for aceita baixa de diferença, registrar settlement explícito de ajuste com motivo/ator, preservando pagamento real. Não zerar saldo ocultamente nem inventar centavo recebido. Preservar a tolerância da janela Demurrage ADR 0015 como regra separada.
...
- [ ] Readiness de comunicação exige todos os B/Ls do cliente/viagem com CE, revisão liberada e financeiro concluído. Avaliar na criação/claim do comunicado, com revisão/lock, e revalidar no envio; não chamar essa leitura de autorização de emissão. Passar identidade de sistema pelo núcleo privado, sem forjar sessão interna no runner.
...
|---|---|---|
| #655 — continuidade da régua | CONTEXT: semanal até quitação, sem limite de semanas. A6, volume simultâneo por cliente, permanece em investigação e não é aceito por esta frase. | Financeiro/Administrativo decide D11 após dimensionar carteira × contatos e limites operacionais |
| #658 F4 — veto sem reprecificação server-side | ADR 0026 aceitou integridade interna do payload. | D02: emissão por worker fora da UI ou decisão de endurecer autoridade; até lá risco explicitamente aberto, cache corrigido |
...
Os riscos aceitos não autorizam chamar itens pendentes de corrigidos. A aceitação de F4 e F6 pode ser revista pelo usuário, mas não foi revogada por este plano. F8 depende de norma vigente; sem essa consulta não declarar “PIX conforme BCB”. F2 depende de prova sob roles reais para qualificar explorabilidade; o contrato inadequado continua no escopo de correção.

## 9. Lacunas e decisões que exigem confirmação na execução

Nenhuma resposta é necessária para salvar este plano. As decisões abaixo são gates das tarefas correspondentes; não constituem pedido para iniciar implementação nesta sessão.
...
| D02 — autoridade do cálculo de Demurrage | Aprovar núcleo server-side com preview versionado, preservando precedência de tarifas/acordos/overrides. Superseder somente parte pertinente da ADR 0026. | Usuário/Financeiro; bloqueia S08-B de reprecificação e worker autônomo de Demurrage, não guards/desconto/cache |
...
| D04 — aviso de bounce e chave global | Escolher e documentar se permanece transacional isento ou segue chave de comunicados; preservar supressão/alerta interno em ambos. | Administrativo; bloqueia mudança de A7, não A1/A3/A9 |
| D05 — centavo local e compartilhamento tardio | Saldo positivo permanece parcial; se aceitar baixa de diferença, criar ajuste explícito. Manter premissa de CE simultâneo até ocorrência/decisão diferente. | Financeiro/Documentação; bloqueia política F14 e automação adicional F6, não prova diagnóstica/impresso |
...
| D11 — volume de cobrança por cliente | Medir faturas elegíveis × contatos por cliente/execução; escolher limite com adiamento justo, comunicação agregada ou manutenção explicitamente aceita. Não adotar número arbitrário nem descartar cobranças acima de um cap. | Financeiro/Administrativo; bloqueia envio em massa de A6, não correções de elegibilidade/inbox |
