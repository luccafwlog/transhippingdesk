...
- [2026-09-12 — Transição de marca: Transhipping Desk → Vela](2026-09-12-plano-transicao-marca-vela.md) — plano de execução da renomeação do sistema interno para Vela, com lançamento direto em `vela.app.br`. Oito blocos ordenados no repositório (B1–B8) e seis itens de painel a cargo do usuário; as quatro decisões de arquitetura estão fechadas. O Portal do Cliente é separado em um segundo build (B6), de modo que a marca interna não exista no artefato entregue ao cliente. A identidade financeira (Transhipping) permanece.
