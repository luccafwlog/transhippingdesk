...
BEGIN
  ...
  SELECT jsonb_build_object(
    ...
    'has_critical_alert', EXISTS (SELECT 1 FROM public.alerts al WHERE al.status <> 'closed' AND al.type IN ('portal_excecao_critica_fatura','portal_convite_expirado','portal_falha_envio','portal_abuso_login') AND ((al.entity_type = 'customer' AND al.entity_id = c.id::text) OR (al.entity_type = 'invoice' AND EXISTS (SELECT 1 FROM public.invoices ai WHERE ai.id = al.entity_id::bigint AND ai.customer_id = c.id)))),
    'has_open_invoice', EXISTS (SELECT 1 FROM public.invoices i WHERE i.customer_id = c.id AND i.status IN ('issued','overdue')),
    'has_active_process', EXISTS (SELECT 1 FROM public.bls b WHERE b.customer_id = c.id AND b.financial_status <> 'cancelled'),
