...
ALTER TABLE public.portal_login_attempts ADD COLUMN IF NOT EXISTS source TEXT NOT NULL DEFAULT 'login'
  CHECK (source IN ('login','recovery'));
...
BEGIN
  ...
  VALUES (encode(extensions.digest(regexp_replace(coalesce(p_login,''),'\D','','g'),'sha256'),'hex'),false,'login');
END; $$;
