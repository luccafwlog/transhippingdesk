-- Renumbered from 20260603192044 (original timestamped migration: 20260603192044_portal_billing_redesign.sql).
-- ============================================================================
-- Portal do cliente — redesenho da lista de faturas (taxas locais)
-- ----------------------------------------------------------------------------
-- Objetivo:
--   - portal_list_invoices: expor os BLs vinculados e o par navio/viagem para
--     a nova coluna de BL e o filtro com autocomplete de navio/viagem.
--   - portal_invoice_details: incluir os containers dos BLs da fatura para o
--     detalhamento (containers + itens cobrados + quantidades).
--
-- Rollback: reverter para as definicoes da migracao
--   20260603130350_portal_auth_uid_rework.sql (mesmas funcoes, sem as colunas
--   bls/vessel_voyages e sem a chave "containers").
-- Sem mudancas de RLS: ambas as funcoes sao SECURITY DEFINER e mantem o filtro
-- por current_portal_customer_id().
-- ============================================================================

-- ----------------------------------------------------------------------------
-- portal_list_invoices() — agora retorna bls[] e vessel_voyages[]
-- ----------------------------------------------------------------------------
...
-- ----------------------------------------------------------------------------
-- portal_invoice_details(p_invoice_id) — agora inclui "containers"
-- ----------------------------------------------------------------------------
...
BEGIN
  ...
  -- Containers dos BLs da fatura (legado via invoice_bls, ledger via links).
  WITH inv_bls AS (
