-- Renumbered from 20260624130000 (original timestamped migration: 20260624130000_demurrage_create_invoice_issued.sql).
-- Demurrage Fase 2.1 — Fatura nasce 'issued' com foto inicial de histórico.
-- Intent: sob recálculo diário não há estágio 'draft'. A invoice é criada já
--   emitida, com o USD travado, o current_total_brl do dia e a primeira linha de
--   demurrage_invoice_history. due_date deixa de existir.
-- Escopo: breaking (assinatura da RPC muda — remove p_due_date, adiciona
--   p_current_roe/p_roe_source). Consumidor: demurrageInvoices.ts.
-- Rollback: restaurar a função de 20260622132732_create_demurrage_invoice_atomic.sql.
