-- ADR 0040: a vigência da Tabela de Taxas Locais deixa de decidir o cálculo.
-- Supersede parcialmente a decisão 3 da ADR 0038 (migrations 266 e 270).
--
-- O que muda:
--
--   1. resolve_local_charge_table_id para de filtrar por valid_from/valid_to.
--      A tabela passa a ser resolvida por escopo (cargo_mode + POD) e por
--      `active`. Entre tabelas ativas do mesmo escopo vence a de vigência
--      inicial mais recente (ORDER BY valid_from DESC, id DESC) -- a vigência
--      continua sendo leitura humana e desempate estável, mas nunca mais
--      EXCLUI uma tabela do cálculo. Desativar a tabela antiga
--      (`active = false`) passa a ser o único jeito de tirá-la do ar.
--
--   2. calculate_bl_local_charges para de exigir ETA. A pendência
--      `review:no_eta` (migration 266) e a trava `IF v_ref_date IS NOT NULL`,
--      que segurava a resolução da tabela e o loop de itens inteiro, deixam de
--      existir: sem ETA o cálculo segue normalmente.
--
--      A data de referência continua existindo, mas só para resolver a
--      Condição de Cliente (customer_rate_overrides), que é acordo negociado
--      com período contratado -- essa vigência permanece valendo (ADR 0038
--      decisão 5, não tocada aqui). A precedência vira ETA da escala do POD ->
--      voyages.eta -> CURRENT_DATE. O último degrau é o que remove a trava:
--      com ele nunca falta data, então o cálculo nunca para por falta dela. É
--      o mesmo critério que o Demurrage já usa (demurrageRates.ts resolve pela
--      vigência do dia do cálculo).
--
--   3. Limpa as pendências `review:no_eta` já gravadas em B/L ainda não
--      faturado -- elas apontam para uma trava que não existe mais. B/L nessa
--      situação não tem linha de item nenhuma (o loop inteiro ficava de fora),
--      então precisa de um recálculo para ganhar valor; a limpeza só evita que
--      a Validação continue mostrando um motivo de bloqueio já revogado.
--
-- Corpo de calculate_bl_local_charges baseado em
-- 270_fix_review_501_faturamento.sql (versão vigente), com as decisões acima
-- aplicadas.

-- ---------------------------------------------------------------------------
-- 1. Resolução da tabela: escopo + active, sem vigência.
-- ---------------------------------------------------------------------------
...
AS $$
  -- p_reference_date é aceito por compatibilidade de assinatura e
  -- deliberadamente ignorado (ADR 0040): a vigência da tabela é informativa.
  SELECT ct.id
...
-- ---------------------------------------------------------------------------
-- 2. Motor de cálculo sem a trava do ETA.
-- ---------------------------------------------------------------------------
...
BEGIN
  ...
  -- bls.financial_status) tambem vive na RPC, para cobrir chamada direta fora
  -- do app.
  ...
  -- "parar e sinalizar" da etapa 7 (aqui o padrao ja e cobrar).
  ...
  -- ADR 0040: a data de referencia deixa de decidir QUAL tabela vale (a
  -- vigencia da tabela virou informativa) e passa a servir apenas para
  -- resolver a Condicao de Cliente, que continua tendo periodo contratado
  -- (ADR 0038 decisao 5). Precedencia: ETA da escala do POD -> voyages.eta ->
  -- CURRENT_DATE. O ultimo degrau e o que remove a trava: com ele nunca falta
  -- data, entao `review:no_eta` (migration 266) deixa de existir e o calculo
  -- nunca mais para por ETA ausente.
  --
  -- O formato ISO continua sendo validado antes do ::DATE porque
  -- pod_schedule_snapshot vem de new_value TEXT de uma tabela de auditoria
  -- (046_voyage_schedule_snapshot_trigger.sql) que nunca valida formato; um
  -- cast cru sobre texto malformado derrubaria o calculo do B/L inteiro (e do
  -- lote, ja que o import chama isso automaticamente). Texto fora do padrao
  ...
  IF v_table_id IS NULL THEN
    -- ADR 0040: a mensagem deixa de citar data de referencia -- nenhuma tabela
    -- e mais recusada por vigencia. O que falta e tabela ativa no escopo.
  ...
  IF v_bl.cargo_mode = 'container' THEN
    ...
    SELECT
      ...
      -- para o loop de itens corrigir o arredondamento por container. O ultimo
      -- B/L do grupo (maior bl_id) absorve a diferenca.
  ...
  -- silencio. Sinaliza uma vez por B/L, nao por item.
  IF v_bl.cargo_mode = 'container' THEN
    SELECT NOT EXISTS(
  ...
  IF v_table_id IS NOT NULL THEN
    FOR item IN
      ...
      LEFT JOIN LATERAL (
        ...
        -- maximo uma linha bate aqui. LIMIT 1 fica como rede de seguranca.
        LIMIT 1
      ) AS ov ON TRUE
    ...
    LOOP
        ...
        IF v_bl.cargo_mode = 'container' THEN
          IF v_is_thd THEN
            ...
            ELSE
              ...
              -- cadastrado -- para e sinaliza em vez de cobrar zero.
              v_auto_review := true;
              INSERT INTO public.charge_calculations (
                bl_id, charge_table_id, charge_item_id, source, status, calculation_key, quantity,
                total_value_brl, review_reason, notes, created_by, calculated_at
      ...
      IF item.application_basis = 'container_distinct_voyage' AND v_bl.cargo_mode = 'container' THEN
        ...
        -- absorvendo a diferenca de arredondamento.
  ...
  SET
    ...
    -- em USD nao seguram mais o faturamento -- a conversao pelo ROE acontece
    -- na emissao (migration 268).
...
-- ---------------------------------------------------------------------------
-- 3. Limpeza das pendencias review:no_eta ja gravadas.
--
-- Só toca B/L ainda não faturado: B/L faturado tem o valor congelado (ADR 0038
-- decisão 2) e não pode ter linha de cálculo mexida por migration. Na prática
-- nenhum B/L nessa situação chegou a faturar (sem ETA não havia linha nenhuma
-- para cobrar), mas a condição mantém a trava explícita.
-- ---------------------------------------------------------------------------
DELETE FROM public.charge_calculations AS cc
USING public.bls AS b
WHERE cc.bl_id = b.id
  AND cc.calculation_key = 'review:no_eta'
  AND COALESCE(b.financial_status, 'pending') NOT IN ('invoiced', 'partially_paid', 'paid');

-- Tira o motivo de bloqueio que citava a trava revogada. O B/L continua em
-- review_required/not_calculated até alguém recalcular (a Validação e a ficha
-- do B/L já oferecem o recálculo); o status correto sai de lá.
UPDATE public.bls
SET billing_hold_reason = NULL
WHERE billing_hold_reason LIKE '%ETA da escala do POD ausente%'
  AND COALESCE(financial_status, 'pending') NOT IN ('invoiced', 'partially_paid', 'paid');

-- Rollback:
--   Reaplicar 270_fix_review_501_faturamento.sql (calculate_bl_local_charges) e
--   a definição de resolve_local_charge_table_id de
--   016_local_charges_stage_a.sql. As pendências review:no_eta apagadas voltam
--   a ser geradas no recálculo seguinte.
