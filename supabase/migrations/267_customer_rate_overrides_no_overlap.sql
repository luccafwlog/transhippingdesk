-- Etapa 10 do plano de faturamento (docs/plans/2026-08-06-faturamento-ajuste-completo.md,
-- ADR 0038 decisão 5, achado 5): duas Condições de Cliente para o mesmo
-- cliente e item não podem ter vigências que se sobrepõem. Até aqui só havia
-- um índice (016_local_charges_stage_a.sql:215) e o motor desempatava com
-- `ORDER BY created_at DESC LIMIT 1` — ou seja, cadastrar uma condição
-- sobreposta não dava erro, só silenciosamente escondia a anterior. Vira
-- restrição real de exclusão no banco.
--
-- Pré-checagem: se já existem sobreposições gravadas, a migration falha com
-- uma mensagem listando os pares em conflito, em vez de uma violação de
-- constraint genérica — resolver esses casos é manual, como o plano pede
-- ("conferir dados existentes antes de aplicar").
--
-- Corpo da função baseado em 266_rate_reference_date_from_pod_eta.sql, com o
-- desempate por created_at removido (a restrição já garante no máximo uma
-- linha elegível).

-- ---------------------------------------------------------------------------
-- 1. Pré-checagem: falha com diagnóstico se houver sobreposição existente.
-- ---------------------------------------------------------------------------
DO $$
DECLARE
  v_conflicts TEXT := '';
...
BEGIN
  FOR v_row IN
    SELECT
      a.id AS id_a, b.id AS id_b, a.customer_id, a.charge_item_id,
      a.valid_from AS from_a, a.valid_to AS to_a, b.valid_from AS from_b, b.valid_to AS to_b
    FROM public.customer_rate_overrides a
    JOIN public.customer_rate_overrides b
      -- Achado da review da PR 501: a EXCLUDE constraint abaixo usa
      -- `customer_id WITH =` / `charge_item_id WITH =`, e em exclusion
      -- constraints NULL nunca conflita consigo mesmo (unknown, nao match).
      -- A pre-checagem tinha IS NOT DISTINCT FROM, mais estrita que a
      -- constraint: abortava a migration em pares NULL=NULL que a propria
      -- restricao sendo instalada aceitaria. Usa `=` para casar a mesma
      -- semantica.
  ...
  LOOP
    v_conflicts := v_conflicts || format(
      'customer_id=%s charge_item_id=%s: id %s (%s a %s) x id %s (%s a %s); ',
      v_row.customer_id, v_row.charge_item_id,
      v_row.id_a, COALESCE(v_row.from_a::TEXT, '-inf'), COALESCE(v_row.to_a::TEXT, '+inf'),
      v_row.id_b, COALESCE(v_row.from_b::TEXT, '-inf'), COALESCE(v_row.to_b::TEXT, '+inf')
    );
  END LOOP;

  IF v_conflicts <> '' THEN
    RAISE EXCEPTION 'customer_rate_overrides tem vigencias sobrepostas; resolva manualmente antes desta migration: %', v_conflicts
      USING ERRCODE = '23505';
  END IF;
END $$;

-- ---------------------------------------------------------------------------
-- 2. Restrição de exclusão: mesmo cliente + item não pode ter vigências que
--    se sobrepõem. NULL em valid_from/valid_to já é tratado como -infinity/
--    +infinity pelo daterange, então "sem limite" também é coberto.
-- ---------------------------------------------------------------------------
CREATE EXTENSION IF NOT EXISTS btree_gist;

ALTER TABLE public.customer_rate_overrides
  ADD CONSTRAINT customer_rate_overrides_no_overlap
  EXCLUDE USING gist (
    customer_id WITH =,
    charge_item_id WITH =,
...
-- ---------------------------------------------------------------------------
-- 3. Remove o desempate agora sem função no motor de cálculo.
-- ---------------------------------------------------------------------------
