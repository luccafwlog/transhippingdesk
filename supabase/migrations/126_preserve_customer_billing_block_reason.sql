-- Renumbered from 20260618145508 (original timestamped migration: 20260618145508_preserve_customer_billing_block_reason.sql).
-- Preserve the specific hold reason produced by run_billing_for_import_batch.
-- A B/L can have no charge rows because customer reconciliation blocked billing,
-- which does not mean that its POD/cargo mode has no valid charge table.
