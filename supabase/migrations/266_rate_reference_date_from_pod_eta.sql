-- Etapa 9 do plano de faturamento (docs/plans/2026-08-06-faturamento-ajuste-completo.md,
-- ADR 0038 decisão 3, achado 4): a data de referência usada para resolver a
-- tabela de taxas locais e a Condição de Cliente deixa de ser
-- uploaded_at/created_at do lote de importação e passa a ser a ETA da escala
-- do POD do B/L (voyages.pod_schedule_snapshot, chave = POD). Duas âncoras
-- alternando faziam o mesmo B/L dar preços diferentes em recálculos
-- diferentes, dependendo de qual delas estava disponível no momento.
--
-- ETA ausente vira pendência de revisão (`review:no_eta`), não fallback para
-- outra data — sem ETA não há tabela nem override resolvidos, e o item loop
-- inteiro fica de fora (gated por `IF v_ref_date IS NOT NULL`). A checagem
-- roda depois da isenção de veículo (etapa 8): B/L isento não precisa de
-- tarifa nenhuma, então não faz sentido cobrar ETA dele.
--
-- Não verificado nesta migration: a taxa de preenchimento real da ETA por
-- escala no banco (o plano pede essa checagem antes de aplicar a etapa; não
-- foi possível confirmar contra dados de produção nesta sessão — decisão
-- registrada pelo usuário: implementar assumindo que a ETA está sempre
-- presente, como a regra operacional declara). Se a taxa de preenchimento
-- real for menor que isso, esta migration vira atrito: B/Ls que hoje
-- calculam passam a cair em revisão por falta de ETA.
--
-- Corpo baseado em 265_vehicle_exemption_requires_lcl_movement.sql.
