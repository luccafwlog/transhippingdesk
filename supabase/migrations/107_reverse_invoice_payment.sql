-- Renumbered from 20260613170000 (original timestamped migration: 20260613170000_reverse_invoice_payment.sql).
...
BEGIN
  ...
  VALUES (
    ...
    'issued',
    v_actor, now(),
    COALESCE(NULLIF(TRIM(COALESCE(p_reason, '')), ''), 'Estorno de pagamento demurrage')
  );
...
REVOKE ALL ON FUNCTION public.reverse_demurrage_payment(BIGINT, TEXT, UUID) FROM PUBLIC;
