-- Etapa 13 do plano de faturamento (docs/plans/2026-08-06-faturamento-ajuste-completo.md,
-- ADR 0038, achado 9): arredondamento do rateio de container compartilhado.
-- calculate_bl_local_charges dividia o valor do item pela quantidade de B/Ls
-- que compartilham o container (1/share_count) e arredondava o total de CADA
-- B/L de forma independente -- 3 B/Ls dividindo um item de R$ 100 pagavam
-- R$ 33,33 cada, somando R$ 99,99 (perde 1 centavo do container). Agora o
-- calculo soma por container, e o ultimo B/L do grupo (maior bl_id, ordem
-- estavel e deterministica -- nao ha outro criterio de desempate definido
-- pelo plano) absorve a diferenca de arredondamento, entao a soma das partes
-- sempre fecha o valor cheio do item por container. Mesma logica aplicada a
-- linhas em BRL e em USD.
--
-- A quantidade fracionaria (1/share_count somada por container, ja existente
-- em v_qty_std/imo/oog/total) continua sendo gravada em `quantity` como
-- valor informativo -- o total em dinheiro (`total_value_brl`/`total_value_usd`)
-- e quem muda de "ROUND(quantidade agregada * tarifa, 2)" para a soma por
-- container com correcao de arredondamento.
