-- Migration 043: Bloquear mark_bl_ready_for_billing quando B/L não tem cliente vinculado.
--
-- Contexto: B/Ls sem customer_id podem chegar a ready_for_billing e ficarem presos —
-- não aparecem na fila de faturamento (sem destinatário) nem retornam a pending.
-- Esta migration adiciona validação explícita com código P0003 para facilitar
-- tratamento diferenciado no frontend.
...
BEGIN
  ...
  SELECT id, charge_status, pod, cargo_mode, customer_id
  INTO v_bl
  FROM public.bls
  WHERE id = p_bl_id;
  ...
  -- Bloquear se não há cliente vinculado
