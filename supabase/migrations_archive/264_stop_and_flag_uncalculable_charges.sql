-- Etapa 7 do plano de faturamento (docs/plans/2026-08-06-faturamento-ajuste-completo.md,
-- ADR 0038 decisão 7): "parar e sinalizar, nunca cobrar zero". Hoje o motor
-- pula em silêncio (CONTINUE) três situações em que ele não sabe calcular a
-- quantidade de um item: application_basis sem implementação (ex.: 'teu'),
-- item THD cadastrado com cargo_profile='any' (o motor só distingue
-- standard/imo/oog) e B/L de container sem nenhum container cadastrado. As
-- três agora gravam uma linha `review_required` com o motivo, no mesmo padrão
-- que a checagem de weight_ton ausente já usava, em vez de simplesmente não
-- cobrar e não avisar ninguém — esse é o achado de maior risco do ADR 0038.
--
-- Corpo baseado em 262_lock_recalc_invoiced_bl.sql, com três adições: guarda
-- "sem containers cadastrados" uma vez por B/L (achado 1); guarda "THD com
-- perfil any" por item; guarda "application_basis não suportada" por item.
-- Nenhuma trava de faturamento existente (financial_status) foi tocada.

-- Etapa 2 do plano de faturamento (docs/plans/2026-08-06-faturamento-ajuste-completo.md,
-- ADR 0038 achado 6): calculate_bl_local_charges recusa recalcular um B/L cujo
-- financial_status ja avancou para faturado, no proprio banco. Ate aqui essa
-- trava so existia em nivel de aplicacao (chargeOperationsService.ts, unico
-- ponto de entrada do app para esta RPC) porque escrever em
-- supabase/migrations/ estava bloqueado nesta sessao; agora que o bloqueio foi
-- levantado, a trava equivalente entra tambem na RPC, cobrindo chamada direta
-- fora do app (mesmo padrao que add_manual_bl_charge ja tem desde a migration
-- 108). A mensagem de erro explica o motivo em vez de falhar em branco.
--
-- Corpo identico ao definido em 151_guard_definer_rpcs_active_user.sql, com
-- duas mudancas: financial_status entra na SELECT do B/L, e um novo bloco logo
-- apos a checagem de "nao encontrado" recusa quando ja faturado/parcialmente
-- pago/pago.
