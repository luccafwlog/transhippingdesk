-- Importacao VAZIOS EXP por (viagem, container), com todos os campos operacionais.
...
BEGIN
  ...
  WITH raw_incoming AS (
    ...
    INSERT INTO public.vazios_bookings (
      ...
      origin_terminal, destination, notes, embark_port, depot, depot_id, material, bundle,
      transporte, hand_in_date, hand_out_date, overtime_handling, overtime_transport, os_number,
      condition, visual_check, overtime_handling_pct, overtime_transport_pct
    )
    SELECT p_voyage_id, v_manifest_id, booking_number, container_number, container_type, movement_date,
      ...
      COALESCE(material, condition = 'material', FALSE), COALESCE(bundle, FALSE), COALESCE(transporte, FALSE),
      hand_in_date, hand_out_date, COALESCE(overtime_handling, FALSE), COALESCE(overtime_transport, FALSE), os_number,
      COALESCE(condition, CASE WHEN material THEN 'material' ELSE 'empty' END), COALESCE(visual_check, FALSE),
      COALESCE(overtime_handling_pct, 0), COALESCE(overtime_transport_pct, 0)
    FROM incoming
    ON CONFLICT (voyage_id, container_number) DO UPDATE SET
      ...
      depot_id = EXCLUDED.depot_id, material = EXCLUDED.material, bundle = EXCLUDED.bundle,
      transporte = EXCLUDED.transporte, hand_in_date = EXCLUDED.hand_in_date,
      hand_out_date = EXCLUDED.hand_out_date, overtime_handling = EXCLUDED.overtime_handling,
      overtime_transport = EXCLUDED.overtime_transport, os_number = EXCLUDED.os_number,
      condition = EXCLUDED.condition, visual_check = EXCLUDED.visual_check, overtime_handling_pct = EXCLUDED.overtime_handling_pct,
      overtime_transport_pct = EXCLUDED.overtime_transport_pct
    RETURNING (xmax = 0) AS inserted
  )
  SELECT count(*) FILTER (WHERE inserted), count(*) FILTER (WHERE NOT inserted)
  INTO v_inserted, v_updated FROM upserted;

  RETURN jsonb_build_object('manifest_id', v_manifest_id, 'total', v_total, 'inserted', v_inserted, 'updated', v_updated);
END;
$function$;
