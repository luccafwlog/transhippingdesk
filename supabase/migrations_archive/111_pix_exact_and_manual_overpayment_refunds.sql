-- Renumbered from 20260614160000 (original timestamped migration: 20260614160000_pix_exact_and_manual_overpayment_refunds.sql).
-- PIX exato + excedente manual vira restituicao
--
-- Regras de negocio:
--  * Conciliacao por PIX (source = 'pix_extract'): o QR copia-e-cola tem valor fixo,
--    entao a baixa precisa quitar exatamente o saldo aberto. Qualquer divergencia
--    (a mais ou a menos) e rejeitada.
--  * Baixa manual (source = 'manual'): o operador informa valor + data. Pode pagar
--    a menos (baixa parcial, ja suportada) ou a maior. No caso a maior, a fatura e
--    quitada e o excedente vira uma restituicao pendente ao cliente (invoice_refunds).
--
-- Rollback: DROP TABLE public.invoice_refunds; e restaurar register_ledger_invoice_payment
-- da migration 20260612162000_register_ledger_partial_payments.sql.

CREATE TABLE IF NOT EXISTS public.invoice_refunds (
  id BIGSERIAL PRIMARY KEY,
  invoice_id BIGINT NOT NULL REFERENCES public.invoices(id) ON DELETE CASCADE,
  -- ON DELETE CASCADE: ao estornar o pagamento (DELETE em payments), a restituicao
  -- gerada por aquele excedente e removida automaticamente.
  ...
  CONSTRAINT invoice_refunds_amount_positive CHECK (amount_brl > 0),
  CONSTRAINT invoice_refunds_status_check CHECK (status IN ('pending', 'settled', 'cancelled'))
);

CREATE INDEX IF NOT EXISTS idx_invoice_refunds_invoice
  ON public.invoice_refunds(invoice_id);

CREATE INDEX IF NOT EXISTS idx_invoice_refunds_pending
  ON public.invoice_refunds(invoice_id)
...
DROP POLICY IF EXISTS invoice_refunds_delete_admin ON public.invoice_refunds;

CREATE POLICY invoice_refunds_select_admin
  ON public.invoice_refunds FOR SELECT
  TO authenticated USING (public.is_admin());

CREATE POLICY invoice_refunds_insert_admin
  ON public.invoice_refunds FOR INSERT
  TO authenticated WITH CHECK (public.is_admin());

CREATE POLICY invoice_refunds_update_admin
  ON public.invoice_refunds FOR UPDATE
  TO authenticated USING (public.is_admin()) WITH CHECK (public.is_admin());

CREATE POLICY invoice_refunds_delete_admin
  ON public.invoice_refunds FOR DELETE
  TO authenticated USING (public.is_admin());
...
BEGIN
  ...
  -- PIX e copia-e-cola de valor fixo: precisa quitar exatamente o saldo aberto.
  -- Sem pagamento parcial e sem excedente por PIX.
  IF COALESCE(p_source, 'manual') = 'pix_extract' AND ABS(p_amount_brl - v_open) > 0.01 THEN
    RAISE EXCEPTION 'Conciliacao PIX exige valor exato: recebido %, saldo aberto %.',
      p_amount_brl, v_open USING ERRCODE = '22023';
  END IF;

  -- Excedente (valor pago acima do saldo aberto). So permitido na baixa manual,
  -- onde vira restituicao ao cliente. Para PIX o bloco acima ja rejeitou.
  v_excess := ROUND(GREATEST(p_amount_brl - v_open, 0)::NUMERIC, 2);
  IF v_excess > 0.01 AND COALESCE(p_source, 'manual') <> 'manual' THEN
  ...
  v_remaining := ROUND(LEAST(p_amount_brl, v_open)::NUMERIC, 2);
...
-- Leitura do total pendente de restituicao de uma fatura (para exibir na UI).
-- SECURITY INVOKER: respeita a RLS admin de invoice_refunds.
CREATE OR REPLACE FUNCTION public.get_invoice_pending_refund(p_invoice_id BIGINT)
RETURNS NUMERIC
LANGUAGE sql
STABLE
SECURITY INVOKER
SET search_path = public, pg_temp
AS $$
  SELECT COALESCE(SUM(amount_brl), 0)::NUMERIC
  FROM public.invoice_refunds
  WHERE invoice_id = p_invoice_id AND status = 'pending';
$$;

REVOKE ALL ON FUNCTION public.get_invoice_pending_refund(BIGINT) FROM PUBLIC;
