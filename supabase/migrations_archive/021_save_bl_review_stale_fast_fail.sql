-- 021_save_bl_review_stale_fast_fail.sql
-- Avoid lock waits when optimistic-lock timestamp is already stale.
...
BEGIN
  ...
  -- Fast path: if timestamp is already stale, fail before trying UPDATE/row lock.
  SELECT updated_at
