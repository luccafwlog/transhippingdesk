-- Renumbered from 20260612155000 (original timestamped migration: 20260612155000_manifest_import_postprocess_transactional.sql).
...
DECLARE
  v_batch_id BIGINT;
  v_bl_ids TEXT[];
  v_error_msg TEXT := 'Falha crítica: Nenhuma tabela de preco ou tarifa encontrada.';
BEGIN
  ...
  WITH bls_without_charges AS (
    SELECT unnest(v_bl_ids) AS id
    EXCEPT
  ...
  SET
    ...
    billing_hold_reason = v_error_msg || ' Adicione os precos e recalcule.',
    notes = CASE
      WHEN COALESCE(b.notes, '') = '' THEN 'Pendencias de importacao: ' || v_error_msg
      WHEN b.notes NOT ILIKE '%' || v_error_msg || '%' THEN b.notes || ', ' || v_error_msg
      ELSE b.notes
    END
  FROM bls_without_charges missing
  WHERE b.id = missing.id
    AND COALESCE(b.charge_status, '') <> 'exempt';
