-- Demurrage — validacao de autoconsistencia por item na emissao.
-- Intent: a formula P1/P2 roda no cliente (calculateDemurrage) e a RPC so
--   conferia SUM(subtotal) vs p_total_usd. O banco passa a rejeitar itens cuja
--   aritmetica interna nao fecha, tornando-se autoridade de veto sobre o
--   calculo enviado pelo browser (revisao de arquitetura 2026-07-17, cand. 4):
--     * subtotal_usd = days_p1*rate_p1_usd + days_p2*rate_p2_usd (+-0.01);
--     * total_days = return_date - discharge_date;
--     * days_p1 + days_p2 <= GREATEST(total_days - free_days, 0)
--       (<= e nao =: um gap configurado entre faixas P1/P2 reduz dias cobrados);
--     * contadores e taxas nao-negativos.
--   Nao re-resolve tarifas da demurrage_rates: o cache TTL do cliente tornaria
--   a emissao nao-deterministica (falso positivo apos edicao de tarifa).
-- Escopo: aditivo no contrato (mesma assinatura); payloads legitimos do
--   frontend atual continuam aceitos — calculateDemurrage satisfaz os
--   invariantes por construcao (paridade coberta em
--   demurrageInvoiceItemConsistency.test.ts).
-- Rollback: reaplicar a definicao da funcao de
--   169_demurrage_invoice_unique_active.sql.
