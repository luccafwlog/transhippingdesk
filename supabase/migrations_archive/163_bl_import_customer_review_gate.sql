-- Keep BL freight import aligned with the customer reconciliation/review gate.
...
DO $$
DECLARE
  v_backfilled_bl_ids TEXT[] := ARRAY[]::TEXT[];
  v_bl_id TEXT;
BEGIN
  WITH matched AS (
    SELECT
    ...
    JOIN public.customers AS c
      ON public.normalize_document_text(c.cnpj_cpf) = public.normalize_document_text(b.manifest_customer_cnpj_cpf)
    WHERE COALESCE(public.normalize_document_text(b.manifest_customer_cnpj_cpf), '') <> ''
      AND COALESCE(b.financial_status, 'pending') <> 'invoiced'
  ...
  SELECT COALESCE(array_agg(id ORDER BY id), ARRAY[]::TEXT[])
  INTO v_backfilled_bl_ids
  FROM updated;

  FOREACH v_bl_id IN ARRAY v_backfilled_bl_ids LOOP
    PERFORM public.sync_customer_reconciliation_queue_for_bl(v_bl_id);
  END LOOP;
END $$;
