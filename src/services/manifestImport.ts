...
export async function importManifest({
  ...
  const blPayload = manifest.bls.map((bl) => {
    ...
    return {
      ...
      // Full positional party blocks persisted for Mercante EDI (M5) generation.
      consignee_block: bl.consignee_block ?? null,
      consignee_address: null,
      consignee_phone: bl.consignee_phone ?? null,
      shipper_block: bl.shipper_block ?? null,
      notify_cnpj_cpf: bl.notify_cnpj_cpf ?? null,
      notify_block: bl.notify_block ?? null,
      notify2_block: bl.notify2_block ?? null,
      total_packages: bl.total_packages ?? null,
      packages_unit: bl.packages_unit ?? null,
