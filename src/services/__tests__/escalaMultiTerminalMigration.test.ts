...
describe('contrato SQL da escala com múltiplos terminais', () => {
  ...
  it('expõe a RPC transacional com revisão, lock, validações, ADR e retorno estável', () => {
    ...
    expect(sql).toMatch(/p_export_expectation[\s\S]+containers_qty[\s\S]+movements_qty[\s\S]+discharge_ports[\s\S]+ce_status[\s\S]+linked/i)
    expect(sql).toMatch(/WHEN COALESCE\(p_export_expectation, '\{\}'::JSONB\) \? 'containers_qty'[\s\S]+ELSE v_old_export_containers_qty/i)
    expect(sql).toMatch(/WHEN COALESCE\(p_export_expectation, '\{\}'::JSONB\) \? 'discharge_ports'[\s\S]+ELSE v_old_export_discharge_ports/i)
