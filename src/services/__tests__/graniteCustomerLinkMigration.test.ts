...
it('separa sugestao e vinculo no Granito sem contaminar o faturamento', () => {
  const sql = readFileSync(resolve(process.cwd(), 'supabase/migrations/286_granite_customer_link_requires_document.sql'), 'utf8')
  ...
  expect(sql).toMatch(/REVOKE ALL ON FUNCTION public\.import_granite_manifest_transactional_legacy_136/i)
  expect(sql).toMatch(/REVOKE ALL ON FUNCTION public\.save_granite_bl_review_legacy_148/i)
})
