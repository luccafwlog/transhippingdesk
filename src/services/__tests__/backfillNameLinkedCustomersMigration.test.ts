...
const sql = readFileSync(resolve(process.cwd(), 'supabase/migrations/283_backfill_name_linked_customers.sql'), 'utf8')
...
describe('backfill de vinculos feitos por nome', () => {
  it('preserva faturados, decisoes humanas e e idempotente', () => {
    ...
    expect(sql).toContain("financial_status, 'pending') <> 'invoiced'")
    ...
    expect(sql).toContain("charge_status, 'not_calculated') <> 'invoiced'")
