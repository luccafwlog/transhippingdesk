...
export async function upsertDepot(input: Omit<Depot, 'id' | 'created_at' | 'updated_at'> & { id?: string }): Promise<void> {
  const payload = { code: input.code.trim(), name: input.name?.trim() || null, pol_port: input.pol_port?.trim() || null, active: input.active, free_time_days: input.free_time_days ?? 0, updated_at: new Date().toISOString() }
