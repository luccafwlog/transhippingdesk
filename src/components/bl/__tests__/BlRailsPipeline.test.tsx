...
describe('BlRailsPipeline', () => {
  it('mostra os dois trilhos e a proxima acao', () => {
    render(<MemoryRouter><BlRailsPipeline operational={[stage('pol', 'Saida do POL', 'done'), stage('pod', 'Chegada ao POD', 'pending')]} financial={[stage('ce', 'CE Mercante', 'pending', 'Cadastrar CE')]} nextAction={{ key: 'ce', label: 'CE Mercante', detail: 'Cadastrar CE', state: 'pending', href: '/manifestos/BL1?tab=detalhes' }} /></MemoryRouter>)
    expect(screen.getByText('Operacional')).toBeTruthy()
    expect(screen.getByText('Financeiro')).toBeTruthy()
    expect(screen.getByText(/Proxima acao/i)).toBeTruthy()
