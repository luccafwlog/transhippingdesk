...
export function BlVisaoGeralTab({ active, bl, cockpit, isContainerMode, containerSummary, breakbulkSummary, onCod, onRestore, disposition, omission, savingDisposition, portalStatus, blDivergenceCount }: {
  ...
  return <div className="grid gap-5 lg:grid-cols-2">
    ...
    <Card><h3 className="mb-3 text-sm font-semibold">Carga</h3>{isContainerMode ? <><dl className="grid gap-2 text-sm sm:grid-cols-3"><Item label="Containers">{String(containerSummary.distinct)}</Item><Item label="IMO">{String(containerSummary.imo)}</Item><Item label="OOG">{String(containerSummary.oog)}</Item></dl><Link to={`/baplie?voyage=${bl.voyage_id}`} className="mt-3 inline-block">{blDivergenceCount ? <Badge tone="yellow">{blDivergenceCount} divergencia(s) Baplie</Badge> : <Badge tone="green">Baplie sem divergencias</Badge>}</Link></> : <dl className="grid gap-2 text-sm sm:grid-cols-3"><Item label="Maquinas">{String(breakbulkSummary.machines)}</Item><Item label="Packages">{String(breakbulkSummary.packagesTotal)}</Item><Item label="Peso (t)">{String(breakbulkSummary.weightTon)}</Item></dl>}</Card>
