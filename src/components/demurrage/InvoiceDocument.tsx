...
export function InvoiceDocument({ detail, type }: Props) {
  ...
  return (
    <div style={documentRoot}>
      ...
      {isInvoice && invoice.pix_payload && <div style={{ marginTop: 20, paddingTop: 16, borderTop: '1px solid #e5e7eb' }}>
        <div style={{ display: 'flex', gap: 18, alignItems: 'flex-start' }}><QRCodeSVG value={invoice.pix_payload} size={90} level="M" /><div style={{ flex: 1, fontSize: '12px', lineHeight: 1.6 }}><div style={{ fontWeight: 700, fontSize: '13px', marginBottom: 4 }}>PAGAMENTO VIA PIX</div><div>Escaneie o QR Code ou utilize o código Pix Copia e Cola.</div><div style={{ marginTop: 4 }}>Valor da fatura: <strong>{fmtBRL(totalBRL)}</strong></div></div></div>
        <div style={{ fontSize: '10px', color: '#6b7280', fontWeight: 600, letterSpacing: '0.05em', margin: '10px 0 3px' }}>PIX COPIA E COLA</div>
        <span style={{ display: 'block', fontFamily: 'monospace', fontSize: '6.5px', background: '#f3f4f6', padding: '5px 8px', whiteSpace: 'nowrap', color: '#374151' }}>{invoice.pix_payload}</span>
      </div>}
      {!isInvoice && <div style={{ textAlign: 'center', margin: '20px 0', fontSize: '32px', fontWeight: 900, color: '#166534', border: '4px solid #166534', borderRadius: 8, padding: '8px 0', letterSpacing: 8 }}>PAGO</div>}
